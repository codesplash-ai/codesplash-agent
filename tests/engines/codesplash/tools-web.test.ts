...
describe("web_fetch SSRF guard", () => {
  ...
  test("blockedAddressClass allows public addresses and handles zones", () => {
    ...
    expect(blockedAddressClass("100.100.100.100")).toBe("a shared-address range")
    expect(blockedAddressClass("224.0.0.1")).toBe("a multicast or reserved address")
    expect(blockedAddressClass("ff02::1")).toBe("a multicast address")
